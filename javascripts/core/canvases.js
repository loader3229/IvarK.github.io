...
var canvas = getEl("studyTreeCanvas");
var ctx = canvas.getContext("2d");
var canvas3 = getEl("dilationCanvas");
var ctx3 = canvas3.getContext("2d");
var bhc = getEl("blackHoleCanvas");
var bhctx = bhc.getContext("2d");
var msc = getEl("studyTreeCanvas2");
var msctx = msc.getContext("2d");
var qkc = getEl("quarkCanvas");
...
function animationOnOff(name) {
    ...
    if (name == "floatingText") getEl("floatingTextAnimBtn").textContent = "Floating text: " + ((player.options.animations.floatingText) ? "ON" : "OFF")
    else if (name == "bigCrunch") getEl("bigCrunchAnimBtn").textContent = "Big crunch: " + (player.options.animations.bigCrunch === "always" ? "ALWAYS" : player.options.animations.bigCrunch ? "ON" : "OFF")
    else if (name == "tachyonParticles") getEl("tachyonParticleAnimBtn").textContent = "Tachyon particles: " + ((player.options.animations.tachyonParticles) ? "ON" : "OFF")
    else if (name == "blackHole") getEl("blackHoleAnimBtn").textContent = "Black hole: " + ((player.options.animations.blackHole) ? "ON" : "OFF")
    else if (name == "quarks") getEl("quarksAnimBtn").textContent="Quarks: O"+(player.options.animations[name]?"N":"FF")
    else if (name == "ghostify") getEl("ghostifyAnimBtn").textContent="Ghostify: O"+(player.options.animations[name]?"N":"FF")
}

function drawAnimations(ts){
    if (player.dilation.tachyonParticles.gte(1) && getEl("eternitystore").style.display !== "none" && getEl("dilation").style.display !== "none" && player.options.animations.tachyonParticles) {
...
function drawTreeBranch(num1, num2) {
    if (getEl("timestudies").style.display === "none") return
    ...
    var start = getEl(num1).getBoundingClientRect();
    var end = getEl(num2).getBoundingClientRect();
...
function drawStudyTree() {
    ...
    if (getEl("secretstudy").style.opacity != "0") drawTreeBranch("11", "secretstudy");
    ...
    if (shiftDown && getEl("eternitystore").style.display !== "none" && getEl("timestudies").style.display !== "none") {
        for (i=0; i<all.length; i++) {
            var start = getEl(all[i]).getBoundingClientRect();
            var x1 = start.left + (start.width / 2) + (document.documentElement.scrollLeft || document.body.scrollLeft);
            var y1 = start.top + (start.height / 2) + (document.documentElement.scrollTop || document.body.scrollTop);
            ctx.fillStyle = 'white';
            ...
            if (getEl(all[i]).className.split(" ")[1] !== undefined || all[i] > 220) {
                var tempName = getEl(all[i]).className.split(" ")[1];
                var name;
